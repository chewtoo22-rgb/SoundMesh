...
enum class SoundQualityMode(val title: String, val subtitle: String, val sampleRate: Int, val bitrateKbps: Int) {
...
enum class AudioProfile(val title: String, val description: String, val tag: String) {
...
data class MasterSystemStats(
    val deviceName: String = "Master Host Phone", val deviceModel: String = "Samsung Galaxy", val ipAddress: String = "127.0.0.1",
    val batteryPercent: Int = 88, val isCharging: Boolean = false, val wifiSignalDbm: Int = -50, val wifiSignalLevel: Int = 4,
    val wifiSsid: String = "SoundMesh Wi-Fi", val wifiLinkSpeedMbps: Int = 433, val masterVolumePercent: Int = 85,
    val isMuted: Boolean = false, val nowPlayingTitle: String = "Spotify / All Apps System Audio",
    val nowPlayingArtist: String = "44.1 kHz 16-bit PCM • Low-Latency Stream", val isPlaying: Boolean = false,
    val audioProfile: AudioProfile = AudioProfile.ALL_STEREO, val latencyMode: LatencyMode = LatencyMode.BALANCED,
    val activeBitrateKbps: Int = 1411, val connectedSpeakersCount: Int = 0, val timestamp: Long = System.currentTimeMillis()
)
...
data class SpeakerDevice(
    val id: String, val name: String, val ipAddress: String, val port: Int = 9876,
    val channel: SpeakerChannel = SpeakerChannel.STEREO_ALL, val zone: SpatialZone = SpatialZone.ALL_PARTY,
    val audioProfile: AudioProfile = AudioProfile.ALL_STEREO, val equalizer: EqualizerSettings = EqualizerSettings(),
    val volume: Float = 1.0f, val isMuted: Boolean = false, val latencyMs: Long = 18, val fineTrimMs: Int = 0,
    val batteryPercent: Int = 88, val isCharging: Boolean = false, val wifiSignalDbm: Int = -52, val wifiSignalLevel: Int = 4,
    val deviceModel: String = "Satellite Phone", val isConnected: Boolean = true, val isSyncLocked: Boolean = true,
    val bufferHealthPercent: Int = 98, val lastSeenTimestamp: Long = System.currentTimeMillis()
)

data class MeshState(
    val role: DeviceRole = DeviceRole.MASTER, val isPlaying: Boolean = false, val masterVolume: Float = 0.85f,
